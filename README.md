...
インストーラーの既定インストール先は `%LOCALAPPDATA%\Programs\yt-dlp-YYY`(標準ユーザー権限のみで書き込み可能、管理者権限不要でインストールできる)。以前はProgram Files配下が既定で、標準ユーザー権限では一時ファイル書き込みに失敗する不具合があったため変更した。
...
13. 現在のバージョンはウィンドウ右下とタイトルバーに表示される(`v1.6.0`)
14. 権限起因で保存・情報取得に失敗する場合は、ウィンドウ左下の「管理者権限で開き直す」ボタンから管理者権限で再起動できる(既に管理者権限で起動している場合は「管理者」表示になりボタンは出ない)
